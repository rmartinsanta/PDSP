...
public class WMPRv5 extends WMPR {

    public WMPRv5(Reconstructive<PDSPSolution, PDSPInstance> repair, Improver<PDSPSolution, PDSPInstance> improver) {
    ...
    public Set<PDSPSolution> generateNewSet(PDSPSolution[] originSolutions, PDSPSolution[] guidingSolutions) {
        ...
        for(var s: guidingSolutions){
            ...
            for (int i = 2; i < randomizedGuides.size(); i++) {
                ...
                var frequencies = freq(realGuides, true);
                var list = walk(s, frequencies);
