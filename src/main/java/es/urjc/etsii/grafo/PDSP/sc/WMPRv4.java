...
public class WMPRv4 extends WMPR {

    private final int k = 5;
    private final int nSubconjuntos = 9;


    public WMPRv4(Reconstructive<PDSPSolution, PDSPInstance> repair, Improver<PDSPSolution, PDSPInstance> improver) {
        super(repair, improver);
    }

    public WMPRv4(CSharpGRGrasp constructive, RemoveNodesLS ls, double evaluationProbability) {
    ...
    public Set<PDSPSolution> generateNewSet(PDSPSolution[] originSolutions, PDSPSolution[] guidingSolutions) {
        ...
        for (var solution : originSolutions) {
            for (int i = 0; i < nSubconjuntos; i++) {
                ...
                var frequencies = freq(pick, true);
